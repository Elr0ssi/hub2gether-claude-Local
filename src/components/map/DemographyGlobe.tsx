...
import { getCountryFillColorDemo, getMaxMetricValueDemo } from "@/lib/demographyColors";

/* ═══════════════════════════════════════════════════════════════════════════
   DEMOGRAPHY GLOBE
   The same machinery as EconomyGlobe — cloned rather than shared, since the
   metric and country-data types differ — painting a deep ocean, land carrying
   the metric's own ramp, white hairline borders, a graticule, a halo and two
   orbital arcs.
...
let REMPLISSAGE: Remplissage = getCountryFillColorDemo;
...
function appliquePalette(p?: PaletteGlobe) {
  ...
  REMPLISSAGE = p?.remplissage ?? getCountryFillColorDemo;
}
...
interface Props {
   ...
   * démographie ne le passe pas et se comporte exactement comme avant.
   */
   ...
   * qui s'applique quand rien n'est passé — la page démographie du site est donc
   ...
   * pixels. Absente, rien n'est mesuré ni appelé : la page démographie du site
...
export function DemographyGlobe({
  demographyYear,
  metric,
  selectedCountry,
  onCountryClick,
  ...
  documentedRef.current = new Set(Object.keys(demographyYear.countries));
  ...
  useEffect(() => {
    ...
    const handle = requestAnimationFrame(() => {
      ...
      const maxValue = getMaxMetricValueDemo(demographyYear.countries, metric);
      ...
      if (satellite) {
        ...
        for (const name of Object.keys(demographyYear.countries)) {
          const feat = byName.get(name);
          if (!feat) continue;
          const fill = REMPLISSAGE(name, demographyYear.countries, maxValue, metric);
  ...
  useEffect(() => {
    ...
    const resize = () => {
      /* La taille de mise en page, pas la taille à l'écran.
         ...
         Hors transformation, les deux mesures coïncident : la page démographie
