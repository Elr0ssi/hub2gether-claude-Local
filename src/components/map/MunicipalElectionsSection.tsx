...
import { ChevronDown, Search, Building2, AlertCircle, Calendar } from "lucide-react";
  ...
  if (round.cities.length === 0) {
    return (
      ...
      >
        ...
        <p style={{ color: "var(--ink-4)", fontSize: "0.72rem" }}>Les résultats seront disponibles après le {round.date.split("-").reverse().join("/")}</p>
      </div>
    );
  }
          ...
          <div className="mt-1">
            ...
            <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "var(--surface)" }}>
              <div
                ...
                style={{ width: `${city.voteShare}%`, background: city.partyColor }}
              />
            </div>
          </div>
        </div>
      ...
      <div className="px-5 py-4 flex flex-col gap-4">
        ...
        {selectedCountry && hasData && data && (
          <>
            ...
            {activeRound && (
              <div>
                <p className="text-xs font-semibold mb-3" style={{ color: "var(--ink-3)" }}>
                  {activeRound.label}
                </p>
                <CityResultsGrid round={activeRound} />
              </div>
            )}
          </>
        )}
