  ...
  return (
    <SlideBody>
      ...
      >
        ...
        {/* Les verbes ne tombent qu'au troisième temps, l'un après l'autre. */}
        <div style={{ opacity: step >= 2 ? 1 : 0, transition: "opacity 0.3s ease" }}>
          {step >= 2 && <VerbStack verbs={S3_MISSING.verbs} startDelay={0.1} />}
        </div>
      </div>
    </SlideBody>
  );
...
function AnswerSlide() {
  ...
  const annee = useMemo(() => getYearData(S3_ANSWER.globe.year), []);
...
function DepthSlide() {
  ...
  /* La mise en page de la V2 est reprise telle quelle : la capture d'article à
     ...
     Ce que la V3 ajoute : l'exemple tenu de bout en bout, et le rôle de chaque
...
function SplitSlide() {
  ...
  const AXE = 150;
  const PAS = 160;
  const largeur = S3_SPLIT.chain.length * PAS;
  ...
  return (
    <SlideBody>
      ...
      >
        {[S3_SPLIT.human, S3_SPLIT.agents].map((c, i) => (
...
function EconomicsSlide() {
  ...
  return (
    <SlideBody>
      ...
      >
        <MarginalCurve
          classicLabel={S3_ECONOMICS.sides[0].label}
          classicDetail={S3_ECONOMICS.sides[0].detail}
          pipelineLabel={S3_ECONOMICS.sides[1].label}
          pipelineDetail={S3_ECONOMICS.sides[1].detail}
          xLabel={S3_ECONOMICS.curve.x}
          yLabel={S3_ECONOMICS.curve.y}
          axisNote={S3_ECONOMICS.curve.caption}
          startDelay={0.5}
          width={1380}
          height={470}
        />
