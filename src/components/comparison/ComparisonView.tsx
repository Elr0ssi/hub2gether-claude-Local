...
import { COMPARISON_COUNTRIES, getComparisonByName } from "@/data/comparison/comparisonData";
...
function ComparisonRow({ label, unit, valueA, valueB, flagA, flagB, nameA, nameB, higherLabel = "plus élevé" }: ComparisonRow & { flagA: string; flagB: string; nameA: string; nameB: string }) {
...
export function ComparisonView() {
  ...
  const economyRows: ComparisonRow[] = [
    { label: "PIB",                    unit: "Mds$",    valueA: ecoA?.gdp         ?? null, valueB: ecoB?.gdp         ?? null, higherLabel: "plus grand" },
    { label: "PIB / habitant",         unit: "USD/hab", valueA: cmpA?.gdp_per_capita_usd ?? null, valueB: cmpB?.gdp_per_capita_usd ?? null, higherLabel: "plus riche" },
    { label: "Dette / PIB",            unit: "%",       valueA: debtA?.debt_2024_pct     ?? ecoA?.debt_ratio ?? null, valueB: debtB?.debt_2024_pct ?? ecoB?.debt_ratio ?? null, higherLabel: "plus endetté" },
    { label: "Dette 2024",             unit: "Mds$",    valueA: debtA ? debtA.debt_2024_bn * 1.09 : null, valueB: debtB ? debtB.debt_2024_bn * 1.09 : null, higherLabel: "plus de dette" },
    { label: "Taux de chômage",        unit: "%",       valueA: ecoA?.unemployment  ?? null, valueB: ecoB?.unemployment  ?? null, higherLabel: "plus de chômage" },
    { label: "Inflation 2024",         unit: "%",       valueA: debtA?.inflation_2024    ?? null, valueB: debtB?.inflation_2024    ?? null, higherLabel: "plus d'inflation" },
    { label: "Entreprises",            unit: "k",       valueA: ecoA?.companies      ?? null, valueB: ecoB?.companies      ?? null, higherLabel: "plus d'entreprises" },
  ];

  const geoRows: ComparisonRow[] = [
    { label: "Superficie",   unit: "km²",     valueA: cmpA?.surface_km2          ?? null, valueB: cmpB?.surface_km2          ?? null, higherLabel: "plus grand" },
    { label: "Population",   unit: "M hab",   valueA: cmpA?.population_millions  ?? null, valueB: cmpB?.population_millions  ?? null, higherLabel: "plus peuplé" },
    { label: "Densité",      unit: "hab/km²", valueA: cmpA ? cmpA.population_millions * 1_000_000 / cmpA.surface_km2 : null, valueB: cmpB ? cmpB.population_millions * 1_000_000 / cmpB.surface_km2 : null, higherLabel: "plus dense" },
  ];

  const resourceRows: ComparisonRow[] = [
    { label: "Production pétrole", unit: "kbd",    valueA: cmpA?.oil_kbd   ?? null, valueB: cmpB?.oil_kbd   ?? null, higherLabel: "plus producteur" },
    { label: "Production gaz",     unit: "Bcm/an", valueA: cmpA?.gas_bcm   ?? null, valueB: cmpB?.gas_bcm   ?? null, higherLabel: "plus producteur" },
    { label: "Production blé",     unit: "Mt/an",  valueA: cmpA?.wheat_mt  ?? null, valueB: cmpB?.wheat_mt  ?? null, higherLabel: "plus producteur" },
    { label: "Production charbon", unit: "Mt/an",  valueA: cmpA?.coal_mt   ?? null, valueB: cmpB?.coal_mt   ?? null, higherLabel: "plus producteur" },
  ];

  const militaryRows: ComparisonRow[] = [
    { label: "Budget militaire", unit: "Mds$", valueA: cmpA?.military_budget_bn ?? null, valueB: cmpB?.military_budget_bn ?? null, higherLabel: "plus dépensier" },
  ];
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <p className="text-xs text-center" style={{ color: "var(--ink-4)" }}>
        Sources : Banque mondiale, FMI, OIT, BP Statistical Review, SIPRI, FAO — données 2023/2024
      </p>
    </div>
  );
}
