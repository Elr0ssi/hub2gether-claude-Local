      ...
      <div className="space-y-2" role="list">
        {items.map((item, i) => {
          ...
          return (
            ...
            >
              <button
                className="w-full text-left px-5 py-4 flex items-start justify-between gap-4 hover:bg-[var(--surface-2)] transition-colors"
                onClick={() => setOpenIndex(isOpen ? null : i)}
                aria-expanded={isOpen}
              >
                <span className="text-heading-3 text-sm font-medium leading-snug" style={{ color: "var(--ink)" }}>
                  {item.question}
                </span>
                <span
                ...
                >
                  {isOpen ? (
                    <Minus size={11} style={{ color: "#0D7A40" }} />
                  ) : (
                    <Plus size={11} style={{ color: "var(--ink-3)" }} />
                  )}
                </span>
              </button>
